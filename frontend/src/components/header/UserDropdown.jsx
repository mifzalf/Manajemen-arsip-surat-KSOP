  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2">
        <img src="/images/user/owner.jpg" alt="User" className="h-11 w-11 rounded-full" />
        <span className="hidden font-medium sm:block">Nama Anda</span>
      ...
      <Dropdown isOpen={isOpen} onClose={() => setIsOpen(false)} className="absolute right-0 mt-4 flex w-[260px] flex-col rounded-2xl border bg-white p-3 shadow-lg">
        <div>
          <span className="block font-medium text-gray-700">Nama Lengkap Anda</span>
          <span className="block mt-0.5 text-sm text-gray-500">email@anda.com</span>
        </div>
